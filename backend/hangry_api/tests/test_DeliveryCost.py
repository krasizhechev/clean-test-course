...
def test_LotsOfItems():
  #Arrange
  order = [SimpleNamespace(quantity=5) for _ in range(3)]
  delivery_distance = 6
  #Act
...
def test_MiddleOfTheRoadItems():
  #Arrange
  order = [SimpleNamespace(quantity=2) for _ in range(3)]
  delivery_distance = 4
  #Act
  cost = Delivery.calculate(order,delivery_distance)
  #Assert
...
def test_LittleItems():
  #Arrange
  order = [SimpleNamespace(quantity=3), SimpleNamespace(quantity=1)]
  del_dist = 2
  #Act
